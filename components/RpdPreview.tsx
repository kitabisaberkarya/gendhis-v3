...
const RpdPreview: React.FC<RpdPreviewProps> = ({ documentId, freeResultHtml, isPaymentSystemEnabled, userName, error, documentPrice, onTopUpRequest }) => {
  ...
  const handleWordDownload = () => {
    ...
    downloadFile(blob, 'RPD_Gendhis.doc');
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-2 sm:p-4 h-[750px] flex flex-col sticky top-24">
      ...
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4 px-2 sm:px-2">Pratinjau Dokumen RPD</h3>
      <div className="relative flex-grow overflow-hidden bg-slate-50 dark:bg-slate-800/50 rounded-md p-4">
        <div className="h-full overflow-y-auto custom-scrollbar-rose">
          <div ref={contentRef} className="bg-white p-8 shadow-md">
            {isLoading && <div className="flex justify-center items-center h-full"><Spinner/></div>}
            {error ? (<ErrorDisplay message={error} />) : !showContent && !isLoading ? (<div className="flex flex-col items-center justify-center h-full text-center text-slate-400 dark:text-slate-500 p-4"><i className="fa-solid fa-chalkboard-user text-5xl mb-4"></i><p>Hasil RPD Anda akan ditampilkan di sini.</p></div>) : (htmlContent && <div className="text-black" dangerouslySetInnerHTML={{ __html: htmlContent }} />)}
          </div>
        </div>
