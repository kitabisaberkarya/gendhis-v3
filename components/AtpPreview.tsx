...
const AtpPreview: React.FC<AtpPreviewProps> = ({ documentId, freeResultHtml, isPaymentSystemEnabled, userName, error, documentPrice, onTopUpRequest }) => {
  ...
  const handleWordDownload = () => {
    ...
    downloadFile(blob, 'ATP_Gendhis.doc');
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-2 sm:p-4 h-[750px] flex flex-col sticky top-24">
       <style>
          {`
            .custom-scrollbar-fuchsia::-webkit-scrollbar { width: 8px; }
            .custom-scrollbar-fuchsia::-webkit-scrollbar-track { background: #f1f5f9; }
            .dark .custom-scrollbar-fuchsia::-webkit-scrollbar-track { background: #1e293b; }
            .custom-scrollbar-fuchsia::-webkit-scrollbar-thumb { background-color: #d946ef; border-radius: 10px; border: 2px solid #f1f5f9; }
            .dark .custom-scrollbar-fuchsia::-webkit-scrollbar-thumb { background-color: #c026d3; border-color: #1e293b; }
          `}
        </style>
      <PaymentStatusModal isOpen={modalState.isOpen} status={modalState.status} errorMessage={modalState.message} onClose={closeModal}/>
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4 px-2 sm:px-2">Pratinjau Dokumen ATP</h3>
      <div className="relative flex-grow overflow-hidden bg-slate-100 dark:bg-slate-900/50 rounded-md p-2">
        <div className="h-full overflow-y-auto custom-scrollbar-fuchsia">
          <div ref={contentRef}>
            {isLoading && <div className="flex justify-center items-center h-full"><Spinner /></div>}
            {error ? (<ErrorDisplay message={error} />) : !showContent && !isLoading ? (<div className="flex flex-col items-center justify-center h-full text-center text-slate-400 dark:text-slate-500 p-4"><i className="fa-solid fa-sitemap text-5xl mb-4"></i><p>Hasil ATP Anda akan ditampilkan di sini.</p></div>) : (htmlContent && <div className="text-black" dangerouslySetInnerHTML={{ __html: htmlContent }} />)}
          </div>
        </div>
